...
func testBuildpackIntegrationStaticImagesNoStacks(t *testing.T, context spec.G, it spec.S) {
	...
	it.Before(func() {
		...
		runImageID = fmt.Sprintf("%s/resolute-static-run-image-no-stacks-%s", RegistryUrl, uuid.NewString())
...
`,
		...
		Expect(archiveToDaemon(staticImagesNoStacks.RunArchive, runImageID)).To(Succeed())
